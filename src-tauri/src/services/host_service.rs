...
use crate::models::{AuthType, Host, PanelType};
...
fn panel_type_to_str(panel: &Option<PanelType>) -> Option<&'static str> {
    match panel {
        Some(PanelType::Bt) => Some("bt"),
        Some(PanelType::OnePanel) => Some("1panel"),
        None => None,
    }
}
...
pub fn insert(conn: &Connection, host: &Host) -> rusqlite::Result<()> {
    conn.execute(
        "INSERT INTO hosts (id, name, address, port, username, auth_type, auth_ref, group_name, tags, panel_type, panel_url, panel_session_ref, created_at, updated_at)
        ...
        params![
            ...
            panel_type_to_str(&host.panel_type),
            host.panel_url,
            host.panel_session_ref,
            host.created_at,
            host.updated_at,
        ],
    )?;
...
pub fn update(conn: &Connection, host: &Host) -> rusqlite::Result<()> {
    ...
    conn.execute(
        "UPDATE hosts SET name=?1, address=?2, port=?3, username=?4, auth_type=?5, auth_ref=?6, group_name=?7, tags=?8, panel_type=?9, panel_url=?10, panel_session_ref=?11, updated_at=?12 WHERE id=?13",
        params![
            host.name,
            host.address,
            host.port,
...
pub fn list(conn: &Connection) -> rusqlite::Result<Vec<Host>> {
    let mut stmt = conn.prepare(
        "SELECT id, name, address, port, username, auth_type, auth_ref, group_name, tags, panel_type, panel_url, panel_session_ref, created_at, updated_at FROM hosts ORDER BY created_at DESC",
    )?;
...
pub fn get(conn: &Connection, id: &str) -> rusqlite::Result<Option<Host>> {
    let mut stmt = conn.prepare(
        "SELECT id, name, address, port, username, auth_type, auth_ref, group_name, tags, panel_type, panel_url, panel_session_ref, created_at, updated_at FROM hosts WHERE id=?1",
    )?;
...
fn row_to_host(row: &rusqlite::Row) -> rusqlite::Result<Host> {
    ...
    let panel_type: Option<String> = row.get(9)?;
    let tags_json: String = row.get(8)?;
    let auth_type = match auth_type.as_str() {
        "key" => AuthType::Key,
        _ => AuthType::Password,
    };
    let panel_type = match panel_type.as_deref() {
        Some("bt") => Some(PanelType::Bt),
        Some("1panel") => Some(PanelType::OnePanel),
        _ => None,
    };
    let tags: Vec<String> =
    ...
    Ok(Host {
        ...
        tags,
        panel_type,
        panel_url: row.get(10)?,
        panel_session_ref: row.get(11)?,
...
mod tests {
    ...
    fn test_crud_roundtrip() {
        ...
        assert_eq!(fetched.panel_type, Some(PanelType::Bt));
        assert_eq!(fetched.panel_url, Some("https://panel.example.com".to_string()));
